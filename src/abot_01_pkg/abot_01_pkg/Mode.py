...
class Mode(Node):
    ...
    def mode_select(self):
        self.get_logger().info('Type: A for auto, T for teleop, O for ostop, E for estop')    # prints txt
        while rclpy.ok():
            ...
            elif user_message == 'o':  # ostop (not in FSM)
                self.get_logger().info('ostop not implemented in FSM yet (use estop or add a mode)')
            elif user_message == 'e':  # estop
                self.pub.publish(String(data='e'))
            else:
                self.get_logger().info('Type: A for auto, T for teleop, O for ostop, E for estop')
